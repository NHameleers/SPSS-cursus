...
* 1. Maak een variabele aan genaamd 'nu', die de datum/tijd op dit moment weergeeft.
COMPUTE nu = $TIME.
EXECUTE.
FORMATS nu (EDATE10).
...
* Oplossing: Kijk in je data naar het nummer dat verschenen is na het beantwoorden van vraag 1.
*  Heb je met FORMATS de weergave al omgezet naar een datum, dan kan je terug naar het cijfer met
*  FORMATS nu (F1.0).

* 3. Reken uit hoeveel maanden zijn verstreken tussen T0_datum en vandaag.
COMPUTE maanden_tussen_T0_en_vandaag = DATEDIFF(vandaag, T0_datum, 'months').
EXECUTE.

* 4. Maak een variabele aan genaamd T2_datum. T2 is anderhalf jaar na T0. Gebruik hiervoor de DATESUM functie.
COMPUTE T2_datum = DATESUM(T0_datum, 18, 'months').
EXECUTE.
FORMATS T2_datum (EDATE10).
...
COMPUTE leeftijd_vandaag_in_maanden = DATEDIFF(vandaag, geboortedatum, 'months').
MEANS leeftijd_vandaag_in_maanden
  /CELLS MAX.
